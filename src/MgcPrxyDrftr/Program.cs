...
using Settings = MgcPrxyDrftrLib.Settings;
using Help = MgcPrxyDrftrLib.Helpers;
using Helpers = MgcPrxyDrftr.lib.Helpers;
...
namespace MgcPrxyDrftr
{
    internal class Program
    {
        ...
        private static HashSet<string> _sheetList = new();
        ...
        private static readonly DraftLibrary DraftLibrary = new();
        ...
        private static async Task Main()
        {
            ...
            DraftLibrary.CleanFolders();

            Console.WriteLine(">> Checking directories...");
            DraftLibrary.CheckAllDirectories();
            ...
            if (DraftLibrary.UseSetList) { ReadAllConfiguredSets(); } else { ReadAllSets(); }
            ...
            if (IsWindows)
            {
                ...
                Help.CheckNanDeck(DraftLibrary.NanDeckPath);
            }
            else
            {
            ...
            //var foo = DownloadAllSetImages(sets["NEO"]);
        ...
        private static void GenerateCubeDraftMini()
        {
            ...
            DirectoryInfo draftDirectory = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.OutputDirectory}\{DraftLibrary.DraftDirectory}\{DateTime.Now:yyyy-MM-ddTHH-mm-ss}");
                ...
                if(isCommand)
                {
                    ...
                    switch(command.ToLower())
                    {
                        ...
                        case "m":
                            if (StateMachine.CurrentState == LoopState.Options)
                            {
                                Settings.PrintCardBacks = !Settings.PrintCardBacks;
                                Settings.Save();
                            }
                            break;
                        default:
        ...
        private static void PrintMenu(LoopState loopState, int startLeftPosition, int startTopPosition)
        {
            ...
            {
                case LoopState.Main:
                    Helpers.Write("B => Draft Booster", startLeftPosition, startTopPosition + 1);
                    Helpers.Write("D => Create Deck", startLeftPosition, startTopPosition + 2);
                    Helpers.Write("S => Add or Remove Sets", startLeftPosition, startTopPosition + 3);
                    Helpers.Write("R => Print Raw List", startLeftPosition, startTopPosition + 4);
                    Helpers.Write("C => Clipboard", startLeftPosition, startTopPosition + 5);
                    Helpers.Write("F => Print Folder", startLeftPosition, startTopPosition + 6);
                    Helpers.Write("O => Options", startLeftPosition, startTopPosition + 7);
                    Helpers.Write("X => Exit", startLeftPosition, startTopPosition + 9);
                    break;
                case LoopState.Options:
                    Helpers.Write($"P => enable / disable automatic printing [{(Settings.AutomaticPrinting ? "enabled" : "disabled")}]", startLeftPosition, startTopPosition + 1);
                    Helpers.Write($"E => enable / disable basic land download [{(Settings.DownloadBasicLands ? "enabled" : "disabled")}]", startLeftPosition, startTopPosition + 2);
                    Helpers.Write($"D => enable / disable prompting for confirmation where drafting booster [{(Settings.PromptForDraftConfirmation ? "enabled" : "disabled")}]", startLeftPosition, startTopPosition + 3);
                    Helpers.Write($"A => enable / disable alternative (new) draft menu [{(Settings.NewDraftMenu ? "enabled" : "disabled")}]", startLeftPosition, startTopPosition + 4);
                    Helpers.Write($"M => enable / disable printing of card backs [{(Settings.PrintCardBacks ? "enabled" : "disabled")}]", startLeftPosition, startTopPosition + 5);
                    Helpers.Write("B => Back", startLeftPosition, startTopPosition + 8);
                    break;
                case LoopState.BoosterDraft:
                    Helpers.Write("A => List all sets", startLeftPosition, startTopPosition + 1);
                    Helpers.Write("L => List downloaded sets", startLeftPosition, startTopPosition + 2);
                    //Helpers.Write("G => Create general draft booster", startLeftPosition, startTopPosition + 3);
                    Helpers.Write("Format: {SetCode}|{HowManyBoosters}", startLeftPosition, startTopPosition + 6);
                    Helpers.Write("B => Back", startLeftPosition, startTopPosition + 8);
                    break;
                case LoopState.DeckCreator:
                    Helpers.Write("A => List all decks", startLeftPosition, startTopPosition + 1);
                    Helpers.Write("L => List downloaded decks", startLeftPosition, startTopPosition + 2);
                    Helpers.Write("B => Back", startLeftPosition, startTopPosition + 8);
                    break;
                case LoopState.DeckManager:
                    Helpers.Write("C => Print Clipboard (later create a deck from it)", startLeftPosition, startTopPosition + 1);
                    Helpers.Write("B => Back", startLeftPosition, startTopPosition + 8);
                    break;
                case LoopState.SetManager:
                    Helpers.Write("Enter set code to add or remove it.", startLeftPosition, startTopPosition + 1);
                    //Helpers.Write("A => Add Set", startLeftPosition, startTopPosition + 1);
                    //Helpers.Write("R => Remove Set", startLeftPosition, startTopPosition + 2);
                    Helpers.Write("L => List Sets", startLeftPosition, startTopPosition + 2);
                    Helpers.Write("B => Back", startLeftPosition, startTopPosition + 8);
                    break;
                case LoopState.RawListManager:
                    Helpers.Write("B => Back", startLeftPosition, startTopPosition + 8);
        ...
        private static async Task<bool> ReadClipboardAndDownload()
        {
            ...
            if (rawCardString != null)
            {
                ...
                // create directory
                DirectoryInfo directory = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.ListDirectory}\{guid}\");
                directory.Create();
            ...
            FileInfo file = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.ScriptDirectory}\{DraftLibrary.DefaultScriptName}.pdf");
            if (file.Exists) { file.MoveTo($@"{DraftLibrary.BaseDirectory}\{DraftLibrary.OutputDirectory}\{DraftLibrary.ListDirectory}\clipboard_{guid}.pdf"); }
        ...
        private static async void LoadDeckList()
        {
            ...
            FileInfo file = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\DeckList.json");
            if(!file.Exists)
            {
                var valid = await Help.DownloadAndValidateFile("https://mtgjson.com/api/v5/DeckList.json", "https://mtgjson.com/api/v5/DeckList.json.sha256", @$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\");
                if(!valid)
                {
                    throw new Exception("Filechecksum is invalid!");
                }
            }
            DeckList = JsonConvert.DeserializeObject<DeckList>(await File.ReadAllTextAsync(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\DeckList.json"));
        }
        ...
        private static async void LoadSetList()
        {
            ...
            FileInfo file = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\SetList.json");
            if (!file.Exists)
            {
                var valid = await Help.DownloadAndValidateFile("https://mtgjson.com/api/v5/SetList.json", "https://mtgjson.com/api/v5/SetList.json.sha256", @$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\");
                if (!valid)
                {
                    throw new Exception("Filechecksum is invalid!");
                }
            }
            SetList = JsonConvert.DeserializeObject<SetList>(await File.ReadAllTextAsync(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\SetList.json"));
        }

        private static void ReadAllDecks() 
        {
            DirectoryInfo deckDirectory = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\{DraftLibrary.DeckDirectory}\");
            if (!deckDirectory.Exists) return;
            var files = deckDirectory.GetFiles("*.json");
            foreach (var file in files)
            {
                var deck = Help.ReadSingleDeck(file.FullName);
        ...
        {
            DirectoryInfo setDirectory = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\{DraftLibrary.SetDirectory}\");
        ...
        private static void ReadAllConfiguredSets()
        {
            ...
            else
            {
                ...
                foreach (var set in Settings.SetsToLoad)
                {
                    ...
                    _ = Settings.CheckLastUpdate(set, @$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}", DraftLibrary.SetDirectory);
                }

                Console.WriteLine(">> Reading files...");
                foreach (var set in Settings.SetsToLoad)
                {
                    FileInfo file = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\{DraftLibrary.SetDirectory}\{set}.json");
                    ...
                    Settings.CheckLastUpdate(set, @$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}", DraftLibrary.SetDirectory);
        ...
        {
            Settings.CheckLastUpdate(setCode, @$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}", DraftLibrary.SetDirectory);
            return ReadSingleSet(setCode);
        }

        private static SetRoot ReadSingleSet(string setCode)
        {
            FileInfo fileInfo = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\{DraftLibrary.SetDirectory}\{setCode.ToUpper()}.json");

            if (!fileInfo.Exists)
            {
                Settings.CheckSetFile(setCode, @$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\", DraftLibrary.SetDirectory);
            ...
            if(json.SelectToken("data").SelectToken("booster") != null && json.SelectToken("data").SelectToken("booster").SelectToken("default") != null)
            {
                foreach (var item in json.SelectToken("data").SelectToken("booster")?.SelectToken("default").SelectToken("boosters")!)
                {
                    foreach (var item1 in item.SelectToken("contents"))
                    {
                        ...
                        if (!_sheetList.TryGetValue(name, out _))
                        {
                            _sheetList.Add(name);
                        }
                    }
        ...
        private static List<Card> GenerateBooster(string setCode, List<string> additionalSetCodes = null)
        {
            List<Guid> boosterCards = new();
            List<CardIdentifiers> boosterCardIdentifier = new();
            var set = Sets[setCode.ToUpper()];
            ...
            {
                foreach (var item in additionalSetCodes.Select(addSetCode => Sets[addSetCode.ToUpper()]).SelectMany(addSet => Enumerable.Where<Card>(addSet.Data.Cards, item => !cards.ContainsKey(item.Uuid) && item.Side is null or Side.A)))
                {
        ...
        private static async Task<object> PrintDeck(string deckName)
        {
            ...
            // create folder
            DirectoryInfo directory = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.DeckDirectory}\{guid}\");
            directory.Create();
            ...
            if (deck == null)
            {
                ...
                if(deckFromList != null)
                {
                    ...
                    var isValid = await Help.DownloadAndValidateFile($"https://mtgjson.com/api/v5/decks/{deckFromList.FileName}.json", $"https://mtgjson.com/api/v5/decks/{deckFromList.FileName}.json.sha256", @$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\{DraftLibrary.DeckDirectory}\");

                    // when file is valid
                    if(isValid)
                    {
                        // read json 
                        var localDeck = JsonConvert.DeserializeObject<DeckRoot>(File.ReadAllText(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.JsonDirectory}\{DraftLibrary.DeckDirectory}\{deckFromList.FileName}.json"));
                        deck = localDeck.Data;
            ...
            foreach (var card in deck.MainBoard) 
            {
                ...
                for (var i = 0; i < card.Count; i++)
                {
                    ...
                    //}
                    _ = await GetImage(card.Identifiers, @$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.DeckDirectory}\{guid}\");
                }
            }
            ...
            // create pdf
            var proc = CreatePdf(guid, @$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.DeckDirectory}\", Settings.AutomaticPrinting);
            if (proc.ExitCode == 0)
            {
                FileInfo file = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.ScriptDirectory}\{DraftLibrary.DefaultScriptName}.pdf");
                if (file.Exists) { file.MoveTo($@"{DraftLibrary.BaseDirectory}\{DraftLibrary.OutputDirectory}\{DraftLibrary.DeckDirectory}\{deck.Name.Replace(' ', '_').ToLowerInvariant()}_{guid}.pdf"); }
            }
        ...
        private static async Task<object> PrintRawList(string listFileName)
        {
            ...
            // read all lines
            var lines = await File.ReadAllLinesAsync(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.FileDirectory}\{listFileName}").ConfigureAwait(false);
            var isLargeList = lines.Length > 90;
            var lineCounter = 0;

            directory = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.ListDirectory}\{guid}\{subGuid}\");
            directory.Create();
            ...
            DirectoryInfo directoryInfo = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.ListDirectory}\{guid}\");
            var count = 1;
            ...
            foreach (var dir in directoryInfo.GetDirectories())
            {
                ...
                {
                    FileInfo file = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.ScriptDirectory}\{DraftLibrary.DefaultScriptName}.pdf");
                    if (file.Exists) { file.MoveTo($@"{DraftLibrary.BaseDirectory}\{DraftLibrary.OutputDirectory}\{DraftLibrary.ListDirectory}\{listFileName.Replace(' ', '_').ToLowerInvariant()}_{guid}_{count}.pdf"); }
                }
                count++;
            }
        ...
        private static bool PrintDirectory(string directoryPath)
        {
            ...
            if (proc.ExitCode == 0)
            {
                FileInfo file = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.ScriptDirectory}\{DraftLibrary.DefaultScriptNameNoGuid}.pdf");
                if (file.Exists) { file.MoveTo($@"{DraftLibrary.BaseDirectory}\{DraftLibrary.OutputDirectory}\{DraftLibrary.ListDirectory}\folder_{Guid.NewGuid()}.pdf"); }
            }
            return true;
        }
        ...
        private static async Task<bool> Draft(string draftString)
        {
            ...
            DirectoryInfo draftDirectory = new(@$"{DraftLibrary.FullDraftOutputPath}{DateTime.Now:yyyy-MM-ddTHH-mm-ss}");
            ...
            //var boosterList = new Dictionary<Guid, List<Card>>();
            //var cardList = new List<Card>();

            ///* Parallel Section Start */
            //Console.WriteLine("Generating Boosters");
            //var foo = Parallel.For(0, boosterCount, (int i) =>
            //{
            //    var booster = GenerateBooster(set?.Code ?? setCode.ToUpper(), moreSets);
            //    boosterList.Add(Guid.NewGuid(), booster);
            //    cardList.AddRange(booster);
            //});

            //Console.WriteLine("Downloading Missing Images");
            //var bar = Parallel.ForEach(cardList, card =>
            //{
            //    _ = GetImage(card.Identifiers,
            //});
            ...
            //Console.WriteLine($@"Start: creating boosters");
            //Parallel.For(1, boosterCount + 1, (int i) =>
            //{
            //    // new booster guid 
            ...
            //    // load images
            //    foreach (var card in booster) { _ = GetImage(card.Identifiers, boosterDirectory.FullName).ConfigureAwait(false); }
            ...
            //});
            //Console.WriteLine($@"Finished: creating boosters");
            ...
            for (var i = 1; i <= boosterCount; i++)
            {
                ...
                // create directory
                DirectoryInfo boosterDirectory = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.BoosterDirectory}\{boosterGuid}\");
                if (!boosterDirectory.Exists) { boosterDirectory.Create(); }
                ...
                // copy card back
                // TODO: consider double sided cards and/or tokens and such stuff
                if (Settings.PrintCardBacks)
                {
                    var back = new FileInfo(@$"{DraftLibrary.BaseDirectory}\images\mtg.back.png");
                    var blank = new FileInfo(@$"{DraftLibrary.BaseDirectory}\images\blank.png");

                    // fill with blank "cards" until we have a full 9 card page 
                    for (var k = 1; k <= (9 - (booster.Count % 9)); k++)
                    {
                        _ = blank.CopyTo(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.BoosterDirectory}\{boosterGuid}\y{k.ToString().PadLeft(3, '0')}.png", false);
                    }

                    for(var j = 1; j <= booster.Count; j++)
                    {
                        _ = back.CopyTo(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.BoosterDirectory}\{boosterGuid}\z{j.ToString().PadLeft(3, '0')}.png", false);
                    }
                }

                _ = Help.CreatePdfDocument(boosterGuid, @$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.BoosterDirectory}");

                FileInfo file = new(@$"{DraftLibrary.BaseDirectory}\{DraftLibrary.TemporaryDirectory}\{DraftLibrary.BoosterDirectory}\{boosterGuid}\{boosterGuid}.pdf");
        ...
        private static async Task<object> Draft()
        {
            ...
            do
            {
                ...
                for (var i = 1; i <= boosterCount; i++)
                {
                    ...
                    if (IsWindows)
                    {
                        var proc = CreatePdf(boosterGuid,  @$"{DraftLibrary.BaseDirectory}\\\{DraftLibrary.OutputDirectory}\\{DraftLibrary.BoosterDirectory}", Settings.AutomaticPrinting);
        ...
        private static Process CreatePdf(string folder, bool print)
        {
            ...
            proc.StartInfo.Arguments = $"/c {DraftLibrary.NanDeckPath} \"{DraftLibrary.BaseDirectory}\\{DraftLibrary.ScriptDirectory}\\{DraftLibrary.DefaultScriptNameNoGuid}.nde\" /[cardfolder]={folder} /createpdf";
        ...
        private static Process CreatePdf(Guid guid, string folder, bool print)
        {
            ...
            proc.StartInfo.CreateNoWindow = true;
            proc.StartInfo.FileName = "cmd.exe";
            proc.StartInfo.Arguments = $"/c {DraftLibrary.NanDeckPath} \"{DraftLibrary.BaseDirectory}\\{DraftLibrary.ScriptDirectory}\\{DraftLibrary.DefaultScriptName}.nde\" /[guid]={guid} /[boosterfolder]={folder} /createpdf";
        ...
        private static async Task<bool> GetImage(string absoluteDownloadUri, string imageName, string imageExtension, string cacheDirectory, string targetBoosterDirectory, bool copyToBoosterDirectory = true)
        {
            // get unique file name guid
            var fileName = $"{Guid.NewGuid()}.png";

            // check for image
            if (copyToBoosterDirectory)
            {
                FileInfo file =
                    new(@$"{cacheDirectory}\{imageName[..1]}\{imageName.Substring(1, 1)}\{imageName}.{imageExtension}");
                if (file.Exists)
                {
                    _ = file.CopyTo($"{targetBoosterDirectory}{fileName}");
                    return true;
                }
            }
            ...
            await Client.DownloadFileTaskAsync(absoluteDownloadUri, @$"{cacheDirectory}\{imageName[..1]}\{imageName.Substring(1, 1)}\{imageName}.{imageExtension}").ConfigureAwait(false);
            ...
            if (copyToBoosterDirectory)
            {
                ...
                _ = newFile.CopyTo($"{targetBoosterDirectory}{fileName}");
                return true;
            }
        ...
        private static async Task<bool> DownloadAllSetImages(SetRoot set, bool forceRedownload = false)
        {
            //var api = new ApiCaller();
            //var scryfallSet = await api.GetSetAsync(set.Data.Code).ConfigureAwait(false);

            // TODO: download set icon
            //scryfallSet.IconSvgUri

            // TODO: download all images

            //await Parallel.ForEachAsync(set.Data.Cards, async (card, token) =>
            //{
            //    _ = await GetImage(card.Identifiers, string.Empty, false).ConfigureAwait(false);
            //}).ConfigureAwait(false);

            //async void DownloadImageAsync(Card card)
            //{
            //    _ = await GetImage(card.Identifiers, string.Empty, false).ConfigureAwait(false);
            //}

            //Parallel.ForEach(set.Data.Cards, DownloadImageAsync);

            foreach (var card in set.Data.Cards)
            {
                _ = GetImage(card.Identifiers, string.Empty, false);
            }

            return true;
        }

        private static async Task<bool> GetImage(ScryfallApi.Client.Models.Card card, string targetDirectory, bool copyToBoosterDirectory = true)
        {
            ...
            {
                _ = await GetImage(card.ImageUris["png"].AbsoluteUri, card.Id.ToString(), "png", @$"{DraftLibrary.FullScryfallCachePath}", targetDirectory, copyToBoosterDirectory);
            }
            else
            {
                if (card.CardFaces != null)
                {
                    foreach (var item in card.CardFaces)
                    {
                        ...
                        // download image
                        _ = await GetImage(uri, $"{card.Id}_{face}", "png", @$"{DraftLibrary.FullScryfallCachePath}", targetDirectory, copyToBoosterDirectory);
                    }
                }
            }
        ...
        private static async Task<bool> GetImage(CardIdentifiers cardIdentifiers, string targetDirectory, bool copyToBoosterDirectory = true)
        {
            // get scryfall card
            var scryfallCard = await Api.GetCardByScryfallIdAsync(cardIdentifiers.ScryfallId);
            
            return await GetImage(scryfallCard, targetDirectory, copyToBoosterDirectory);
        }

        //private static async Task<bool> GetImage(ScryfallApi.Client.Models.Card card)
        //{
        //    // get unique file name guid
        //    var fileName = $"{Guid.NewGuid()}.png";
        //    var imageName = string.Empty;
        //    var imageExtension = string.Empty;
        //    var cacheDirectory = @$"{BaseDirectory}\{CacheDirectory}\{ScryfallCacheDirectory}";
